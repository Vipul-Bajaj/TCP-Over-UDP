...
public class Go_Back_N_Sender {
	...
	public static void sendFile(String hostName, int portNumber, String fileName) throws IOException {
		System.out.println("Sending "+ fileName +" to the reciever using Go Back N ARQ protocol for error and flow control.");
		...
		boolean isLastAcknowledgedFlag = false;
				...
				if ((sequenceNumber - windowSize) > lastAckedSequenceNumber) {
					...
					while (!ackRecievedCorrect) {
						...
						if (ackPacketReceived) {
							...
							System.out.println("Ack recieved for Sequence Number = " + ackSequenceNumber);
							...
							for (int y=0; y != (sequenceNumber - lastAckedSequenceNumber); y++) {
								...
								DatagramPacket resendPacket = new DatagramPacket(resendMessage, resendMessage.length, address, portNumber);
								udpSocket.send(resendPacket);
		...
		// Continue to check and resend until we receive final acknowledgement
		while (!isLastAcknowledgedFlag) {
			...
			while (!ackRecievedCorrect) {
				...
				if (ackPacketReceived) {		
					...
					ackRecievedCorrect = true;
					break; // Break if there is an acknowledgement so the next packet can be sent
				} else {
					// Resend the packet following the last acknowledged packet and all following that (cumulative acknowledgement)
